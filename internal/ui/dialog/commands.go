...
func (c *Commands) setCommandItems(commandType CommandType) {
	...
	case MCPPrompts:
		for _, cmd := range c.mcpPrompts {
			...
			commandItems = append(commandItems, NewCommandItem(c.com.Styles, "mcp_"+cmd.Name, cmd.PromptID(), "", action))
		}
	}
...
func (c *Commands) defaultCommands() []*CommandItem {
	...
	cfg := c.com.Config()
	agents := cfg.Agents()
	if agentCfg, ok := agents[config.AgentCoder]; ok {
		providerCfg := cfg.GetProviderForModel(agentCfg.Model)
		model := cfg.GetModelByType(agentCfg.Model)
		if providerCfg != nil && model != nil && model.CanReason {
			models := cfg.Models()
			selectedModel := models[config.SelectedModelTypeLarge]
	...
	if c.hasSession {
		cfg := c.com.Config()
		agents := cfg.Agents()
		agentCfg := agents[config.AgentCoder]
		model := cfg.GetModelByType(agentCfg.Model)
