...
func NewModels(com *common.Common, isOnboarding bool) (*Models, error) {
	...
	var err error
	m.providers, err = config.GetProviders(m.com.Config())
	if err != nil {
...
func (m *Models) setProviderItems() error {
	...
	knownProviders, err := config.GetProviders(cfg)
	if err != nil {
	...
	for id, p := range cfg.Providers().Seq2() {
		...
		if !slices.ContainsFunc(knownProviders, containsProviderFunc(id)) ||
			...
			for _, model := range p.Models {
				...
				item := NewModelItem(t, provider, catwalkModel, m.modelType, false)
	...
	for _, provider := range m.providers {
		...
		if providerConfigured {
			...
			for _, model := range providerConfig.Models {
				...
				displayProvider.Models = append(displayProvider.Models, catwalk.Model{
					ID:   model.ID,
					Name: model.Name,
				})
	...
	if len(recentItems) > 0 {
		...
		var validRecentItems []string
		for _, recentStr := range recentItems {
			parts := strings.Split(recentStr, ":")
			if len(parts) < 2 {
				continue
			}
			providerID := parts[0]
			...
			validRecentItems = append(validRecentItems, recentStr)
			recentGroup.AppendItems(item)
			if modelID == currentModel.Model && providerID == currentModel.Provider {
				selectedItemID = item.ID()
			}
		}
