...
func todoList(sessionTodos []session.Todo, spinnerView string, t *styles.Styles, width int) string {
	todoItems := make([]tools.TodoItem, len(sessionTodos))
	for i, t := range sessionTodos {
	...
	return chat.FormatTodosList(t, todoItems, spinnerView, width)
}
...
func (m *UI) togglePillsExpanded() tea.Cmd {
	...
	hasPills := hasIncompleteTodos(m.session.GetTodos()) || m.promptQueue > 0
	if !hasPills {
		return nil
	}
	m.pillsExpanded = !m.pillsExpanded
	if m.pillsExpanded {
		if hasIncompleteTodos(m.session.GetTodos()) {
...
func (m *UI) switchPillSection(dir int) tea.Cmd {
	...
	hasIncompleteTodos := hasIncompleteTodos(m.session.GetTodos())
...
func (m *UI) pillsAreaHeight() int {
	...
	if m.pillsExpanded {
		if m.focusedPillSection == pillSectionTodos && hasIncomplete {
			pillsAreaHeight += len(m.session.GetTodos())
...
func (m *UI) renderPills() {
	...
	var pills []string
	if hasIncomplete {
		pills = append(pills, todoPill(m.session.GetTodos(), inProgressIcon, todosFocused, m.pillsExpanded, t))
	}
	if hasQueue {
	...
	if m.pillsExpanded {
		if todosFocused && hasIncomplete {
			expandedList = todoList(m.session.GetTodos(), inProgressIcon, t, contentWidth)
		} else if queueFocused && hasQueue {
			if m.com.App != nil && m.com.App.AgentCoordinator != nil {
				queueItems := m.com.App.AgentCoordinator.QueuedPromptsList()
