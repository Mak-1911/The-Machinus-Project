...
import (
	...
	"github.com/machinus/cloud-agent/internal/ui/list"
...
func (m *Chat) applyHighlightRange(idx, selectedIdx int, item list.Item) list.Item {
	if hi, ok := item.(list.Highlightable); ok {
		// If mouse is not down, clear any existing highlight
		if !m.mouseDown {
			hi.SetHighlight(-1, -1, -1, -1)
			return hi.(list.Item)
		}

		// Apply highlight for drag selection
		startItemIdx, startLine, startCol, endItemIdx, endLine, endCol := m.getHighlightRange()

		// Don't apply highlight if there's no valid range
		...
		if idx >= startItemIdx && idx <= endItemIdx {
			if idx == startItemIdx && idx == endItemIdx {
				// Single item selection - only apply if there's an actual range
				if startLine == endLine && startCol == endCol {
					// Zero-length selection, don't highlight
					hi.SetHighlight(-1, -1, -1, -1)
					return hi.(list.Item)
				}
				sLine = startLine
		...
		// Set highlight or clear it if no valid selection
		if sLine >= 0 || sCol >= 0 || eLine >= 0 || eCol >= 0 {
			hi.SetHighlight(sLine, sCol, eLine, eCol)
			return hi.(list.Item)
		}

		// Clear highlight if we didn't set anything
		hi.SetHighlight(-1, -1, -1, -1)
		return hi.(list.Item)
	}
