...
import (
	...
	"github.com/machinus/cloud-agent/internal/ui/anim"
	"github.com/machinus/cloud-agent/internal/ui/app"
	"github.com/machinus/cloud-agent/internal/ui/attachments"
	"github.com/machinus/cloud-agent/internal/ui/chat"
	"github.com/machinus/cloud-agent/internal/ui/commands"
	"github.com/machinus/cloud-agent/internal/ui/common"
	"github.com/machinus/cloud-agent/internal/ui/completions"
	"github.com/machinus/cloud-agent/internal/ui/config"
	"github.com/machinus/cloud-agent/internal/ui/dialog"
	"github.com/machinus/cloud-agent/internal/ui/fsext"
	"github.com/machinus/cloud-agent/internal/ui/history"
	"github.com/machinus/cloud-agent/internal/ui/home"
	fimage "github.com/machinus/cloud-agent/internal/ui/image"
	"github.com/machinus/cloud-agent/internal/ui/logo"
	"github.com/machinus/cloud-agent/internal/ui/message"
	"github.com/machinus/cloud-agent/internal/ui/permission"
	"github.com/machinus/cloud-agent/internal/ui/pubsub"
	"github.com/machinus/cloud-agent/internal/ui/session"
	"github.com/machinus/cloud-agent/internal/ui/styles"
	agenttools "github.com/machinus/cloud-agent/internal/ui/tools"
	"github.com/machinus/cloud-agent/internal/ui/tools/mcp"
	"github.com/machinus/cloud-agent/internal/ui/util"
	"github.com/machinus/cloud-agent/internal/ui/version"
)
...
// layoutHelpers provides helper functions for layout operations.
type layoutHelpers struct{}

// Fixed creates a length constraint for a fixed size.
func (layoutHelpers) Fixed(size int) layout.Constraint {
	return layout.Len(size)
}

// SplitVertical splits an area vertically into two parts.
// Returns (top, bottom) where top has the specified height.
func (layoutHelpers) SplitVertical(area uv.Rectangle, topHeight layout.Constraint) (uv.Rectangle, uv.Rectangle) {
	split := layout.Vertical(topHeight, layout.Fill(1)).Split(area)
	if len(split) >= 2 {
...
// SplitHorizontal splits an area horizontally into two parts.
// Returns (left, right) where left has the specified width.
func (layoutHelpers) SplitHorizontal(area uv.Rectangle, leftWidth layout.Constraint) (uv.Rectangle, uv.Rectangle) {
	split := layout.Horizontal(leftWidth, layout.Fill(1)).Split(area)
	if len(split) >= 2 {
		return split[0], split[1]
	}
	return area, uv.Rectangle{}
...
type (
	...
	// assistantResponseMsg is sent when the LLM responds.
	assistantResponseMsg struct {
		sessionID string
		content   string
	}

	// toolProgressMsg is sent when a tool starts or completes execution.
	toolProgressMsg struct {
		...
		result    string
		isError   bool
		isStart   bool // true for tool_start, false for tool_complete
	}
...
type UI struct {
	...
	// Agent execution state for progress tracking
	agentProgressCh    chan app.ProgressEvent
	agentResponseCh    chan string
...
// InitializeSession creates a new session and transitions to chat state.
// This can be called externally to auto-start a chat session.
func (m *UI) InitializeSession() tea.Cmd {
	var cmds []tea.Cmd
	if !m.hasSession() {
		...
		if newSession != nil && newSession.ID != "" {
...
func (m *UI) loadMCPrompts() tea.Cmd {
	return func() tea.Msg {
		prompts, err := commands.LoadMCPPrompts(m.com.Config())
		if err != nil {
...
func (m *UI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case loadSessionMsg:
		...
		if hasInProgressTodo(m.session.GetTodos()) {
	...
	case assistantResponseMsg:
		// Handle the assistant's response from the LLM
		if msg.content != "" {
			asstMsgID := fmt.Sprintf("msg-%d", time.Now().UnixNano())
			asstMsg := message.NewMessage(asstMsgID, message.Assistant, msg.content)
			asstMsg.SetSessionID(msg.sessionID)
			m.com.App.Messages.Add(asstMsg)

			// Only append the assistant message item - tool items were already added via progress events
			asstMsgItem := chat.NewAssistantMessageItem(m.com.Styles, asstMsg)
			m.chat.AppendMessages(asstMsgItem)
		}

		// Clear pending tool items - they've been completed and displayed
		m.pendingToolItems = make(map[string]chat.ToolMessageItem)

		// Clear the prompt queue indicator
		if m.promptQueue > 0 {
			m.promptQueue = 0
			m.updateLayoutAndSize()
		}
	...
	case toolProgressMsg:
		// Handle tool execution progress updates
		if msg.isStart {
			// Tool is starting - create a pending tool item with spinner
			toolCall := message.ToolCall{
				ID:       msg.toolID,
				Name:     mapToolName(msg.toolName),
				Input:    msg.toolArgs,
				Finished: false, // Not finished - will show spinner automatically
			}

			// Create tool item without result (pending state)
			toolItem := chat.NewToolMessageItem(m.com.Styles, "", toolCall, nil, false)

			// Track this item so we can update it when it completes
			m.pendingToolItems[msg.toolID] = toolItem

			// Add to chat (animation will start automatically because Finished=false)
			m.chat.AppendMessages(toolItem)
		} else {
			// Tool is completing - update with result
			if toolItem, exists := m.pendingToolItems[msg.toolID]; exists {
				...
				toolItem.SetResult(toolResult)

				// Update the tool call to mark it as finished
				...
				if msg.isError {
					toolItem.SetStatus(chat.ToolStatusError)
				} else {
					toolItem.SetStatus(chat.ToolStatusSuccess)
				}

				// Stop tracking this tool
				delete(m.pendingToolItems, msg.toolID)
		...
		// Schedule another check for more progress events
		cmds = append(cmds, func() tea.Msg { return checkAgentProgressMsg{} })

	case checkAgentProgressMsg:
		// Poll for agent progress events
		if m.agentProgressCh != nil {
			select {
			case event := <-m.agentProgressCh:
				// Forward progress event as a Bubbletea message
				switch event.Type {
				case "tool_start":
			...
			case response := <-m.agentResponseCh:
				// Agent completed successfully - clean up
				m.com.App.AgentCoordinator.SetProgressCallback(nil)
				close(m.agentProgressCh)
				m.agentProgressCh = nil
				m.agentResponseCh = nil
				m.agentErrorCh = nil

				// Return the assistant response (tool items were already added via progress events)
				cmds = append(cmds, func() tea.Msg {
			...
			default:
				// No events available, schedule another check shortly
	...
	case mcpPromptsLoadedMsg:
		m.mcpPrompts = make([]commands.MCPPrompt, len(msg.Prompts))
		dia := m.dialog.Dialog(dialog.CommandsID)
		if dia == nil {
			break
		}
	...
	case pubsub.Event[session.Session]:
		...
		if m.session != nil && msg.Payload().ID == m.session.ID {
			prevHasInProgress := hasInProgressTodo(m.session.GetTodos())
			sess := msg.Payload()
			m.session = &sess
			if !prevHasInProgress && hasInProgressTodo(m.session.GetTodos()) {
				m.todoIsSpinning = true
				cmds = append(cmds, m.todoSpinner.Tick)
				m.updateLayoutAndSize()
			}
	...
	case pubsub.Event[message.Message]:
		...
		if hasInProgressTodo(m.session.GetTodos()) && m.isAgentBusy() && !m.todoIsSpinning {
			m.todoIsSpinning = true
			cmds = append(cmds, m.todoSpinner.Tick)
		}
	...
	case pubsub.Event[app.LSPEvent]:
		m.lspStates = m.com.App.GetLSPStates()
	...
	case spinner.TickMsg:
		...
		if m.state == uiChat && m.hasSession() && hasInProgressTodo(m.session.GetTodos()) && m.todoIsSpinning {
			var cmd tea.Cmd
...
func (m *UI) setSessionMessages(msgs []*message.Message) tea.Cmd {
	...
	toolResultMap := chat.BuildToolResultMap(msgs)
	if len(msgs) > 0 {
...
func (m *UI) appendSessionMessage(msg message.Message) tea.Cmd {
	...
	existing := m.chat.MessageItem(msg.GetID())
	if existing != nil {
...
func (m *UI) handleChildSessionMessage(event pubsub.Event[message.Message]) tea.Cmd {
	...
	if len(payload.ToolCalls()) == 0 && len(payload.ToolResults()) == 0 {
		return nil
	}
	...
	for _, tc := range payload.ToolCalls() {
		...
		if !found {
			...
			nestedItem := chat.NewToolMessageItem(m.com.Styles, payload.ID(), tc, nil, false)
...
func (m *UI) handleDialogMsg(msg tea.Msg) tea.Cmd {
	...
	case dialog.ActionToggleThinking:
		cmds = append(cmds, func() tea.Msg {
			...
			if err := cfg.UpdatePreferredModel(config.SelectedModelTypeLarge, currentModel); err != nil {
				return util.ReportError(err)()
			}
	...
	case dialog.ActionPermissionResponse:
		...
		case dialog.PermissionAllow:
			m.com.App.Permissions.Grant(msg.Permission.Permission.ID)
		case dialog.PermissionAllowForSession:
			m.com.App.Permissions.GrantPersistent(msg.Permission.Permission.ID)
		case dialog.PermissionDeny:
			m.com.App.Permissions.Deny(msg.Permission.Permission.ID)
		}
			...
			default:
				...
				if msg.String() == "@" && !m.completionsOpen {
					...
					if curIdx == 0 || (curIdx > 0 && isWhitespace(curValue[curIdx-1])) {
						...
						// TODO: get depth and limit from config
						depth, limit := 3, 10
...
func (m *UI) View() tea.View {
	...
	// Use the actual window dimensions if available, otherwise use defaults
	// Note: These defaults are only used before the first WindowSizeMsg arrives
	w := m.width
	h := m.height
	if w <= 0 {
		w = 120
	...
	if h <= 0 {
		h = 100  // Large default to ensure canvas covers full terminal
	}
...
func (m *UI) ShortHelp() []key.Binding {
	...
	case uiChat:
		...
		case uiFocusMain:
			...
			if m.pillsExpanded && hasIncompleteTodos(m.session.GetTodos()) && m.promptQueue > 0 {
...
func (m *UI) generateLayout(w, h int) uiLayout {
	...
	// Add app margins
	appRect, helpRect := layoutSplit.SplitVertical(area, layoutSplit.Fixed(area.Dy()-helpHeight))
	appRect.Min.Y += 1
	appRect.Max.Y -= 1
	helpRect.Min.Y -= 1
	appRect.Min.X += 1
	...
	case uiOnboarding, uiInitialize:
		...
		headerRect, mainRect := layoutSplit.SplitVertical(appRect, layoutSplit.Fixed(landingHeaderHeight))
	...
	case uiChat:
		// Always include header (1 line at top)
		...
		if m.isCompact {
			...
			sessionDetailsArea, _ := layoutSplit.SplitVertical(appRect, layoutSplit.Fixed(detailsHeight))
			...
			if pillsHeight > 0 {
				...
				chatRect, pillsRect := layoutSplit.SplitVertical(mainRect, layoutSplit.Fixed(mainRect.Dy()-pillsHeight))
			...
			mainRect, sideRect := layoutSplit.SplitHorizontal(remainingRect, layoutSplit.Fixed(remainingRect.Dx()-sidebarWidth))
			// Add padding left
			sideRect.Min.X += 1
			mainRect, editorRect := layoutSplit.SplitVertical(mainRect, layoutSplit.Fixed(mainRect.Dy()-editorHeight))
			mainRect.Max.X -= 1 // Add padding right
...
// mapToolName maps tool names from our tool executor to UI tool names.
// The tool executor uses simple names like "bash", "list_files", "grep", "glob"
// but the UI expects specific names like "bash", "ls", "grep", "glob"
func mapToolName(name string) string {
	switch name {
	case "bash", "shell":
		return agenttools.BashToolName
	case "read_file":
		return agenttools.ViewToolName
	case "write_file":
		return agenttools.WriteToolName
	case "list_files":
		return agenttools.LSToolName
	case "grep":
		return agenttools.GrepToolName
	case "glob":
		return agenttools.GlobToolName
	default:
		// For unknown tools, try to use as-is
		return name
	}
}
...
func (m *UI) sendMessage(content string, attachmentList ...message.Attachment) tea.Cmd {
	...
	userMsg := message.NewMessage(fmt.Sprintf("msg-%d", time.Now().UnixNano()), message.User, content)
	userMsg.SetSessionID(m.session.ID)
	m.com.App.Messages.Add(userMsg)

	// Add user message to chat view immediately
	attachmentRenderer := attachments.NewRenderer(
	...
	userMsgItem := chat.NewUserMessageItem(m.com.Styles, userMsg, attachmentRenderer)
	m.chat.AppendMessages(userMsgItem)
	...
	progressCh := make(chan app.ProgressEvent, 100)
	responseCh := make(chan string, 1)
	errCh := make(chan error, 1)

	// Store channels in UI struct for polling
	m.agentProgressCh = progressCh
	m.agentResponseCh = responseCh
	m.agentErrorCh = errCh
	m.agentSessionID = sessionID

	// Set up the progress callback
	m.com.App.AgentCoordinator.SetProgressCallback(func(event app.ProgressEvent) {
		select {
	...
	go func() {
		response, err := m.com.App.AgentCoordinator.Run(context.Background(), sessionID, content, attachmentList...)
		if err != nil {
			errCh <- err
			return
		}
...
func (m *UI) openCommandsDialog() tea.Cmd {
	...
	hasTodos := hasSession && hasIncompleteTodos(m.session.GetTodos())
...
func (m *UI) openPermissionsDialog(perm permission.PermissionRequest) tea.Cmd {
	...
	// TODO: implement Options.TUI.DiffMode
	var opts []dialog.PermissionsOption
	_ = m.com.Config().Options()
...
func handleMCPToolsEvent(cfg *config.UIConfig, name string) tea.Cmd {
	return func() tea.Msg {
		mcp.RefreshTools(
			context.Background(),
			cfg,
